...
Class ProfessorDAO {
	...
	public function __construct($con) {
		$this->con = $con;
		$this->superdao = new SuperDAO('professor');
	}

	//cadastrar
	function cadastrar (professor $obj) {
		$this->sql = sprintf("INSERT INTO professor(nome, datanascimento)
		VALUES('%s', '%s')",
	...
	//atualizar
	function atualizar (Professor $obj) {
		$this->sql = sprintf("UPDATE professor SET nome = '%s', datanascimento = '%s', dataedicao = '%s' WHERE id = %d ",
			mysqli_real_escape_string($this->con, $obj->getNome()),
			mysqli_real_escape_string($this->con, $obj->getDatanascimento()),
	...
	//buscarPorId
	function buscarPorId (Professor $obj) {
		$this->sql = sprintf("SELECT * FROM professor WHERE id = %d",
			mysqli_real_escape_string($this->con, $obj->getId()));
	...
	function listar () {
		...
		if(!$result) {
			$this->superdao->setMsg( resolve( mysqli_errno( $this->con ), mysqli_error( $this->con ), 'Professor' , 'Listar' ) );
		}else{
			while($row = mysqli_fetch_object($result)) {
	...
	function listarPaginado($start, $limit) {
		$this->sql = "SELECT * FROM professor limit " . $start . ", " . $limit;
		$result = mysqli_query ( $this->con, $this->sql );

		$this->superdao->resetResponse();

		if ( !$result ) {
			$this->superdao->setMsg( resolve( mysqli_errno( $this->con ), mysqli_error( $this->con ), 'Professor' , 'ListarPaginado' ) );
		}else{
	...
	//deletar
	function deletar (Professor $obj) {
		$this->superdao->resetResponse();
		...
		$this->sql = sprintf("DELETE FROM professor WHERE id = %d",
			mysqli_real_escape_string($this->con, $obj->getId()));
		$result = mysqli_query($this->con, $this->sql);
	...
	//quantidade total
	function qtdTotal() {
		$this->sql = "SELECT count(*) as quantidade FROM professor";
		$result = mysqli_query ( $this->con, $this->sql );
		if (! $result) {
